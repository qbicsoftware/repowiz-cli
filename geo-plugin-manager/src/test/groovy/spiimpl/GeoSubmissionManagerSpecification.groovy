...
class GeoSubmissionManagerSpecification extends Specification {
    ...
    def "maps repoWizTerm submission model to geoTerm submission model"() {
        ...
        then:
        res.samples.get(0).sampleProperties.sort() == model2.samples.get(0).sampleProperties.sort()
        assert res.project.projectProperties.sort() == model2.project.projectProperties.sort()
    }
